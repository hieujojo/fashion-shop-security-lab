# Khái niệm: Quy trình kiểm thử bảo mật web (1 engagement)
...
### 1. Scope & Rules — làm rõ "được phép làm gì"
...
> Điểm ghi điểm khi phỏng vấn: bạn **tự giác** phân biệt lab vs hệ thống thật — đúng đạo đức
> nghề (nhiều ứng viên thiếu).

### 2. Recon / Mapping — app có những gì
...
Lab:      6 pages → /api/login, /api/products?q=&category=, /api/products/:id,
          /api/products/:id/reviews, /api/profile(, /email), /api/admin/*
          (xem agent/MODULES.md)
```

### 3. Passive Scan — nghe, không đụng
...
### 4. Active Scan — chủ động bắn payload
...
### 5. Manual Verification — bước quyết định chất lượng (lab tập trung nhất)
...
### 6. Triage — từ "nhiều alert" tới "finding thật"
...
### 7. Report + Remediation — sản phẩm tester giao
...
## Sai lầm phổ biến của người mới (tránh — cũng là điều interview hay hỏi)
